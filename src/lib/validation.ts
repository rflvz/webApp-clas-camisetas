...
export function validateClusteringParams(
  ...
  // Convertir valores vacíos, null o 0 a undefined para validación correcta
  // Pero mantener 0 como valor válido si se proporciona explícitamente
  const cleanedParams: Partial<ClusteringParameters> = {};
  for (const [key, value] of Object.entries(params)) {
    if (value === '' || value === null) {
      cleanedParams[key as keyof ClusteringParameters] = undefined;
    } else if (value === 0 && key === 'minClusterSize') {
      // minClusterSize = 0 es inválido, mantenerlo para que Zod lo detecte
      cleanedParams[key as keyof ClusteringParameters] = 0 as any;
    } else if (typeof value === 'string' && !isNaN(Number(value)) && value.trim() !== '') {
      const numValue = Number(value);
      cleanedParams[key as keyof ClusteringParameters] = numValue as any;
    } else if (typeof value === 'number') {
      cleanedParams[key as keyof ClusteringParameters] = value as any;
    } else {
      cleanedParams[key as keyof ClusteringParameters] = value as any;
    }
  }
  
  const result = schema.safeParse(cleanedParams);
  ...
  // Validaciones adicionales de dependencias (estas son errores, no warnings)
  if (cleanedParams.minSamples !== undefined && cleanedParams.minClusterSize !== undefined) {
    if (cleanedParams.minSamples > cleanedParams.minClusterSize) {
      validationResult.isValid = false;
      if (!validationResult.errors.minSamples) {
        validationResult.errors.minSamples = [];
      }
      validationResult.errors.minSamples.push(
        'minSamples no puede ser mayor que minClusterSize. Esto generará solo outliers.'
      );
      validationResult.suggestions.push(
        `Establece minSamples a ${cleanedParams.minClusterSize} o menos para permitir clusters.`
      );
    } else if (cleanedParams.minSamples === cleanedParams.minClusterSize) {
      validationResult.warnings.push(
        'minSamples es igual a minClusterSize. Esto puede generar clusters muy estrictos.'
      );
      validationResult.suggestions.push(
        `Considera establecer minSamples a ${Math.max(1, cleanedParams.minClusterSize - 1)} para mayor flexibilidad.`
      );
    }
  }
  
  // Warnings para configuraciones subóptimas
  if (cleanedParams.minClusterSize !== undefined) {
    if (cleanedParams.minClusterSize < 5) {
      validationResult.warnings.push(
        'Un minClusterSize muy pequeño (< 5) puede generar clusters poco significativos.'
      );
    } else if (cleanedParams.minClusterSize > 100) {
      validationResult.warnings.push(
